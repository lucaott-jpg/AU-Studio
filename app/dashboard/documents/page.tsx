...
interface Document {
  ...
  deals?: { name: string; company: string }
...
function DocumentsInner() {
  ...
  async function fetchAll() {
    ...
    const [{ data: docs }, { data: br }, { data: dl }] = await Promise.all([
      supabase.from('documents').select('*, brands(name,primary_color), deals(name,company), profiles(full_name,email)').order('updated_at', { ascending: false }),
      supabase.from('brands').select('id,name,primary_color,secondary_color,logo_url'),
      supabase.from('deals').select('id,name,company,stage,deal_type').order('name')
    ])
    setDocuments(docs || [])
    setBrands(br || [])
    setDeals(dl || [])
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50/50">
                ...
                <th className="text-left px-4 py-3 text-xs text-gray-400 font-medium uppercase tracking-wider">Deal</th>
