...
const brandColors = [
  { name: 'AU Yellow', hex: '#F5C842', bg: '#F5C842', border: false },
  { name: 'Studio Black', hex: '#0A0A0A', bg: '#0A0A0A', border: false },
  { name: 'Pure White', hex: '#FFFFFF', bg: '#FFFFFF', border: true },
  { name: 'Surface', hex: '#F7F7F5', bg: '#F7F7F5', border: true },
]
        ...
        <div className="grid grid-cols-3 gap-5">
              ...
              <div className="flex flex-col gap-2">
                {workspaces.map(w => (
                  <a key={w.label} href={w.href}
                    className={`bg-white border border-gray-200 p-4 flex items-center gap-4 hover:border-aurum-black transition-colors ${w.accent ? 'border-l-4 border-l-aurum-yellow' : ''}`}>
                    <div className={`w-9 h-9 flex items-center justify-center flex-shrink-0 font-bebas text-sm tracking-wide ${w.accent ? 'bg-aurum-yellow text-aurum-black' : 'bg-aurum-black text-white'}`}>
                      {w.key}
                    </div>
              ...
              <div className="flex flex-col gap-2">
                {recentProjects.map(p => (
                  <div key={p.name} className="bg-white border border-gray-200 px-4 py-3 flex items-center gap-3">
                    <span className={`text-xs font-medium px-2 py-1 border tracking-wide flex-shrink-0 ${typeStyles[p.type]}`}>{p.type}</span>
                    <span className="flex-1 text-sm text-aurum-black">{p.name}</span>
                    <span className="text-xs text-gray-400">{p.team}</span>
                    <span className={`text-xs px-2 py-1 border flex-shrink-0 ${p.statusStyle}`}>{p.status}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="flex flex-col gap-5">
            <div className="bg-white border border-gray-200 p-4">
              <div className="section-label">Claude AI insights</div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {quickCreate.map(q => (
                  <a key={q.label} href={q.href}
                    className={`border p-3 text-xs font-medium text-center cursor-pointer transition-all ${q.accent ? 'border-aurum-yellow bg-yellow-50 text-yellow-800 hover:bg-aurum-yellow hover:text-aurum-black' : 'border-gray-200 text-gray-500 hover:border-aurum-black hover:text-aurum-black'}`}>
                    {q.label}
                  </a>
                ))}
              </div>
            ...
            <div className="bg-white border border-gray-200 p-4">
              ...
              {brandColors.map(c => (
                <div key={c.hex} className="flex items-center gap-3 mb-2">
                  <div className={`w-8 h-8 flex-shrink-0 ${c.border ? 'border border-gray-200' : ''}`} style={{ background: c.bg }} />
                  <div className="flex-1">
                    <div className="text-xs font-medium text-aurum-black">{c.name}</div>
                    <div className="text-xs text-gray-400 font-mono">{c.hex}</div>
                  </div>
                  <button onClick={() => handleCopy(c.hex)} className="text-xs text-aurum-yellow font-medium hover:underline">
